...
def process_input(inp_type, valid_inputs=[], invalid_inputs=[]):
    ...
    if (
        choice in valid_inputs or len(valid_inputs) == 0
    ) and choice not in invalid_inputs:
        return choice
    else:
...
def ops_menu():
    ...
    if inp == 1:
        ...
        if new_size > 9:
            print(
                'Board sizes over 9 will cause bugs with how the game board is drawn. Do you wish to continue? (y/n)\n'
            )
